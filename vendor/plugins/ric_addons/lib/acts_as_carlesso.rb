  ...
  module Acts #:nodoc:
    module Carlesso #:nodoc:
      ...
      module ClassMethods
        ...
        def names
          find(:all).map{|r| r.name rescue r.to_s }
        end
        
        def idnames
          find(:all).map{|record| [ record.id, record.to_s ] }
        end
...
ActiveRecord::Base.send(:include, ActiveRecord::Acts::Carlesso)
