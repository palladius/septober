...
Add the gem to your Gemfile.
  ...
  gem "ricky-generators", :group => :development

Then you can run any of the included generators.

  rails g ricky:scaffold Recipe name:string index new

=== Rails 2

First install the gem.

  gem install ricky-generators
...
The generators will be available in all Rails applications. To run the generator, go to your rails project directory and call it using the script/generate or script/destroy command.
  ...
  script/generate ricky_scaffold Recipe name:string index new
...
* ricky:layout: generates generic layout, stylesheet, and helper files.
* ricky:scaffold: generates a controller and optional model/migration.
* ricky:config: generates a config YAML file and loader.
* ricky:authentication: generates user model with sign up and log in.

To view the README for each generator, run it with the +help+ option.

  rails g ricky:layout --help


== Troubleshooting and FAQs

<b>What is the difference between ricky:scaffold and built-in scaffold?</b>

One of the primary differences is that ricky:scaffold allows you to choose which controller actions to generate.

  rails g ricky:scaffold post name:string index new edit
...
Try running ricky:layout, that will generate this helper method. Or you can just change the templates to whatever approach you prefer for setting the title.
...
If you are having a problem with Ricky Generators, first look at the FAQs above. If you still cannot resolve it, please submit an issue here.

http://github.com/ryanb/ricky-generators/issues
