module TodosHelper
  ...
  def render_agent_first_row_icon(todo)
    ...
    icon = todo.user.resolved_agent_icon
    host = todo.user.agent_host.presence || 'cloud'
    name = todo.user.username.split('.').last.capitalize
    content_tag(:span, "🤖 #{icon}".html_safe, class: 'agent_icon_inline', title: "Autonomous Agent: #{name} on #{host}", style: "margin-right: 4px;")
  end
  ...
  def render_agent_second_row_badge(todo)
    ...
    host = todo.user.agent_host.presence || 'cloud'
    name = todo.user.username.split('.').last.capitalize
    content_tag(:span, " [#{icon} #{name} @ #{host}]".html_safe, class: 'agent_badge_secondary', style: "background: #eef2ff; color: #4338ca; padding: 2px 6px; border-radius: 4px; font-size: 0.75rem; margin-right: 6px; font-weight: 600;", title: "Created by agent #{todo.user.username} on #{host}")
  end

  def render_todo_name(todo)
    title = todo.description
    klasses = "todo #{todo.done? ? 'done' : 'undone' }"
    agent_icon = render_agent_first_row_icon(todo)
    content_tag((todo.active ? :b : :s), "#{agent_icon}#{todo.name}".html_safe, title: title, class: klasses)
  end
  ...
  def render_todo_second_row(todo)
    ret = []
    ret << render_agent_second_row_badge(todo) if todo.user&.agent?
