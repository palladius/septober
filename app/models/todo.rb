  ...
  def apply_todo_regex_magic
    ...
    begin
      ...
      unless attribute_present?('project_id')
        project = Project.find_by(name: 'personal', user_id: self.user_id) ||
                  (user&.parent_id.present? ? Project.find_by(name: 'personal', user_id: user.parent_id) : nil) ||
                  (user&.parent_id.present? ? Project.find_by(user_id: user.parent_id) : nil) ||
                  Project.find_by(user_id: self.user_id) ||
                  Project.create!(name: 'inbox', user_id: self.user_id)
        if project
          log << "AssignedProject: #{project.name} (id: #{project.id})"
